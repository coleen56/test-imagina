...
// fonction qui prend en paramètre une cellule et retourne ses voisines latérales vides sous forme d'un array (non inclusion des cellules voisines diagonales)
function getCasesVoisinesVides(Cellule $cell) {
    // récupération de la variable globale champs 
    global $champs;
    $x = $cell->getX();
    $y = $cell->getY();
    // valeur d'une case à -1 permet de la déclarer "visitée" donc non vide et ne contenant pas de menhir
    $champs[$x][$y] = -1;
    $voisinesVides = [];
...
function getNiveauPropagation() {
    global $start;
    // la seule cellule touchée au tout départ est la cellule de départ de la propagation : $start
